...
const getSubPixelSpacingAndXYDirections = (viewport, subPixelResolution) => {
    ...
    if (viewport instanceof core_1.StackViewport) {
        ...
        const absViewUpDotI = Math.abs(gl_matrix_1.vec3.dot(viewUp, iVector));
        const absViewUpDotJ = Math.abs(gl_matrix_1.vec3.dot(viewUp, jVector));
        const absViewUpDotK = Math.abs(gl_matrix_1.vec3.dot(viewUp, kVector));
        let ySpacing;
